...
const dir = resolve("user", "mods", "EntryPoint");
const filepath = resolve(dir, "log.txt");
writeFileSync(filepath, "");
function logHelper(logger: ILogger, url: string, info: any, output: string): any {
      ...
      [
        {
          ...
          action: (url: string, info: any, sessionID: string, output: string): any => {
            ...
            try {
              const config = JSON.parse(readFileSync(resolve(dir, "config", "config.json"), "utf8"));
              const SpawnPointParams = location.SpawnPointParams.filter(spp => {
                return !spp.Categories.includes('Player') || spp.Id === config.id;
              });
